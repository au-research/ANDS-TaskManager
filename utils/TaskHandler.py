...
def update_slack_channel(message, tasks_info, log_level):
    slack_util = SlackUtils(myconfig.slack_channel_webhook_url, myconfig.slack_channel_id)
    slack_util.post_message(message, tasks_info, log_level)
...
class TaskHandler:
    ...
    def __init__(self, tasks_info, logger, database):
        self.startUpTime = int(time.time())
        self.tasksInfo = tasks_info
        self.logger = logger
        self.database = database
        self.update_tasks_info()

    def update_tasks_info(self, update_message=False):
        ...
        while attempts < 3:
            try:
                ...
                cur = conn.cursor()
                up_time = int(time.time()) - self.startUpTime
                status_dict = {'status': self.__status,
                               'message': self.message,
                               'error': {'log': str.strip(self.errorLog), 'errored': self.errored},
                               'completed': str(self.completed),
                               'output': {'file': self.outputFilePath, 'dir': self.outputDir},
                               'progress': {'time': str(up_time), 'start': str(self.startUpTime), 'end': ''}
                               }
                if update_message:
                    cur.execute("UPDATE %s SET `status` ='%s', `message` = '%s' where `id` = %s" % (
                        myconfig.tasks_table, self.__status, json.dumps(status_dict).replace("'", "\\\'"),
    ...
    def get_info(self):
        ...
        return "STATUS: %s, UP TIME: %s, METHOD: %s, TASK ID: %s, NAME: %s " % (
            self.__status, str(up_time), self.tasksInfo['type'], str(self.tasksInfo['task_id']), self.tasksInfo['name'])
    ...
    def check_run_time(self):
        if self.stopped:
            return
        up_time = int(time.time()) - self.startUpTime
        if up_time > myconfig.max_up_seconds_per_task:
